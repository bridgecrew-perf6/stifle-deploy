...
CREATE TABLE transactions_mode_individual(
   ID INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
   trans_mode VARCHAR(500) NOT NULL
   
);
CREATE TABLE source_fund_individual(
   ID INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
   source_fund VARCHAR(500) NOT NULL
   
);
CREATE TABLE customer_type_organization(
   ID INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
   Cust_Type VARCHAR(500) NOT NULL,
   Risk_Category VARCHAR(20),
   Risk_Weight INT
